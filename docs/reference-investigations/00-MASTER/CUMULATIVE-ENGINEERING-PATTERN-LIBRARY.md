...
# Stirling-PDF observations — 2026-09-07

## PAT-100 — Native work gets an explicit runtime budget
**Observed in:** Stirling `ProcessExecutor`.  
**Purpose:** hostile input/native bugs cannot hold a worker forever.  
**InvoiceIQ:** YES — apply directly to Tesseract via pytesseract timeout.

## PAT-101 — Long-lived lease ownership must be renewable
**Observed by comparison:** Stirling resource ownership lasts for native process lifetime; InvoiceIQ durable leases currently do not renew.  
**Purpose:** distinguish "slow but alive" from "worker died."  
**InvoiceIQ:** YES — guarded job heartbeat.

## PAT-102 — Heartbeat renewal must prove ownership
**Purpose:** a stale owner must not renew a job another worker owns or a terminal row.  
**Recommended predicate:** id + RUNNING + locked_by.  
**InvoiceIQ:** YES.

## PAT-103 — Heartbeat uses a separate persistence session
**Purpose:** do not concurrently use the handler's AsyncSession while handler work is still active.  
**InvoiceIQ:** YES.

## PAT-104 — Native concurrency limits belong to the scarce resource
**Observed in:** Stirling per-process semaphores.  
**InvoiceIQ:** CONDITIONALLY — worker lanes already serve this role until measured otherwise.

## PAT-105 — Temporary resources have an owner and recovery cleanup
**Observed in:** TempFileManager/TempFile + scheduled/startup cleanup.  
**InvoiceIQ:** FUTURE when native temp paths become material.

## PAT-106 — Adaptive memory/disk processing uses input size plus current resource state
**Observed in:** CustomPDFDocumentFactory.  
**InvoiceIQ:** FUTURE; current upload cap is small.

## PAT-107 — Large serialization should avoid double-peak memory
**Observed in:** Stirling temp-backed save-to-bytes.  
**InvoiceIQ:** CONDITIONALLY for large archive/PDF generation.

## PAT-108 — Batch parallelism must be bounded and cleanup partial success
**Observed in:** Stirling concurrent document factory operations.  
**InvoiceIQ:** YES principle; current worker count/lanes are the preferred mechanism.

## PAT-109 — Expand archives only through one hardened bounded extractor
**Observed in:** ZipExtractionUtils.  
**InvoiceIQ:** NO CURRENT NEED because archives are rejected.

## PAT-110 — Automation eligibility is a declared capability
**Observed in:** Stirling tool metadata / automation support.  
**InvoiceIQ:** YES FUTURE; reinforces bounded action allowlists.

## PAT-111 — Validate a workflow step before execution
**Observed in:** PipelineProcessor.  
**InvoiceIQ:** YES; already present in bounded automation/action validation.

## PAT-112 — Multi-step work gets a run correlation identity
**Observed in:** Stirling AutomationRunContext.  
**InvoiceIQ:** YES; reinforces prior Twenty agent/run identity principles.

## PAT-113 — Optional native capabilities degrade explicitly
**Observed in:** Stirling dependency-aware features.  
**InvoiceIQ:** YES; already used for OCR/ClamAV/provider seams.

## PAT-114 — Build variants need matrix tests only when variants are real products
**Observed in:** Stirling core/proprietary/saas CI matrix.  
**InvoiceIQ:** CONDITIONALLY; no fake matrix.

## PAT-115 — CI should prove test artifacts were produced
**Observed in:** Stirling backend workflow explicitly checks test report directories.  
**InvoiceIQ:** CONDITIONALLY for critical generated artifacts/gates.

## PAT-116 — Immutable CI action refs are a cross-repository consensus pattern
**Observed in:** Stirling backend CI; earlier Paperless analysis.  
**InvoiceIQ:** YES — existing action-pinning backlog remains valid.

## PAT-117 — Resource limits live in central configuration
**Observed in:** Stirling process session/timeouts.  
**InvoiceIQ:** YES — OCR timeout goes in `Settings`.

## PAT-118 — Retry policy should be a typed operator outcome, not raw exception prose
**Observed in InvoiceIQ and reinforced by native timeout integration.  
**InvoiceIQ:** YES — `processing_timeout` joins capture-failure vocabulary.

## New rejected patterns

### NO-046 — Generic PDF-processing platform architecture for a finance SaaS
Different product.

### NO-047 — Internal loopback HTTP as default service orchestration
Reuse domain services directly unless HTTP is itself the boundary under test.

### NO-048 — Generic no-code file workflow graph before a product requirement
Bounded financial automation remains safer.

### NO-049 — Archive extraction when archive intake can simply be rejected
Smaller attack surface wins.

### NO-050 — A second native-process concurrency layer without measurements
Worker lanes and deployment worker count already bound concurrency.

### NO-051 — Adaptive temp/disk infrastructure under a small enforced upload cap without memory evidence
Measure first.

### NO-052 — Open-core build flavors without multiple distribution products
Do not create packaging architecture for hypothetical editions.

### NO-053 — Mark domain work `running` as a substitute for durable lease renewal
Crash recovery would then skip legitimate reclaimed work.

### NO-054 — Increase stale lease instead of heartbeating
Moves the race and slows crash recovery; does not solve ownership semantics.

### NO-055 — Generic native ProcessExecutor for one Tesseract integration
Use pytesseract's existing timeout/termination seam first.

---

# Flask observations — 2026-09-07

## PAT-119 — Setup registration has a terminal phase
**Observed in:** Flask `setupmethod`, app/Blueprint setup guards.  
**Purpose:** workers must not see divergent runtime topology.  
**InvoiceIQ:** YES principle; route topology gate now, generic registry freeze only when needed.

## PAT-120 — Deferred module setup is composition, not runtime dynamism
**Observed in:** Flask Blueprints.  
**Purpose:** modules can declare behavior independently, then bind once.  
**InvoiceIQ:** ALREADY via APIRouter; do not recreate Blueprint.

## PAT-121 — Request-local state has explicit push/pop ownership
**Observed in:** Flask AppContext/ContextVar.  
**InvoiceIQ:** ALREADY via FastAPI dependencies + ContextVars.

## PAT-122 — Transport-free core improves reuse and testing
**Observed in:** Flask sans-I/O layer.  
**InvoiceIQ:** ALREADY via framework-independent services/AppError.

## PAT-123 — Extension state is app-specific, not captured in one global extension instance
**Observed in:** Flask extension `init_app` guidance.  
**InvoiceIQ:** FUTURE if third-party plugins become real.

## PAT-124 — Cryptographic purposes need separate keys
**Observed by comparison:** Flask can rotate one session-signing purpose; InvoiceIQ's `secret_key` also derives local KEK material.  
**InvoiceIQ:** YES — separate JWT signing from app/local-KEK secret.

## PAT-125 — Sign current, verify bounded previous keys
**Observed in:** Flask `SECRET_KEY_FALLBACKS`.  
**InvoiceIQ:** YES for routine internal-JWT rotation.

## PAT-126 — Rotation compatibility must preserve independent revocation
**Purpose:** accepting an old signature must not bypass logout/password-reset/session revocation.  
**InvoiceIQ:** YES — jti/live-session remains after decode.

## PAT-127 — Route topology is a structural invariant
**Observed by comparison:** Flask freezes setup; InvoiceIQ already has manual route-order rule.  
**InvoiceIQ:** YES — CI guard.

## PAT-128 — Reuse framework-compiled routing semantics in topology tests
**Purpose:** avoid a second buggy route parser.  
**InvoiceIQ:** YES — inspect Starlette `path_regex`.

## PAT-129 — Warnings become a maintenance gate after inventory
**Observed in:** Flask pytest warnings-as-errors.  
**InvoiceIQ:** YES, RATCHETED — remove blanket ignore after warning inventory.

## PAT-130 — Dependency compatibility matrix should match distribution contract
**Observed in:** Flask minimum/current/dev dependency tests.  
**InvoiceIQ:** NO minimum matrix; exact-pinned SaaS has a different contract.

## PAT-131 — Stage breaking public contract changes
**Observed in:** Flask compatibility wrappers + deprecation window.  
**InvoiceIQ:** YES for public API/provider/automation contracts.

## PAT-132 — Error response finalization runs on failure paths too
**Observed in:** Flask `handle_exception` -> `finalize_request`.  
**InvoiceIQ:** ALREADY through middleware/exception boundary.

## PAT-133 — Global configuration freezes after startup in multiworker systems
**Observed in:** Flask extension guidance.  
**InvoiceIQ:** YES principle.

## PAT-134 — App factory is a tool for multi-app requirements, not a quality badge
**Observed in:** Flask ecosystem.  
**InvoiceIQ:** NO CURRENT NEED.

## PAT-135 — Null-object degradation is valid only when inert behavior is safe
**Observed in:** Flask NullSession.  
**InvoiceIQ:** CONDITIONAL; never for mandatory security controls.

## PAT-136 — Test helpers should model the real application lifecycle
**Observed in:** Flask test client/context helpers.  
**InvoiceIQ:** YES selectively; existing ASGI/domain fixtures already do much of this.

## PAT-137 — Immutable CI action pins are cross-repository consensus
**Observed in:** Flask, Stirling, PSC/Paperless cycles.  
**InvoiceIQ:** YES; existing supply-chain backlog.

## PAT-138 — Workflow permissions default to none/read-only
**Observed in:** Flask test workflow.  
**InvoiceIQ:** YES; existing supply-chain principle.

## PAT-139 — Minimum-version tests are library-specific
**Observed in:** Flask.  
**InvoiceIQ:** NO for exact-pinned deployment dependencies.

## PAT-140 — Namespace module registrations to make collision semantics explicit
**Observed in:** nested Blueprint dotted names.  
**InvoiceIQ:** CONDITIONALLY through router/domain namespaces.

## PAT-141 — Stable machine domain codes beat raw exception taxonomy at SaaS boundaries
**Observed by comparison:** Flask generic error hierarchy vs InvoiceIQ AppError.  
**InvoiceIQ:** KEEP OUR APPROACH.

## New rejected patterns

### NO-056 — Rewrite FastAPI application to Flask
No product problem solved.

### NO-057 — Migrate an ASGI SaaS to WSGI because the reference is mature
Wrong direction for current architecture.

### NO-058 — Add proxy-global request/app APIs over explicit FastAPI dependencies
Reduces typing and auditability.

### NO-059 — Reimplement Flask Blueprint on top of APIRouter
Duplicate abstraction.

### NO-060 — Replace server-side revocable sessions with signed-cookie sessions
Weaker fit for privileged finance access.

### NO-061 — Rotate a shared app secret while it also derives local KEK material
Can break sealed-secret decryptability.

### NO-062 — Retain a known-compromised signing key as a verification fallback
Compromise rotation requires immediate retirement + session revocation.

### NO-063 — App-factory refactor without a real multi-app requirement
High churn, weak benefit.

### NO-064 — Freeze registries solely because they are technically mutable
Need evidence of runtime mutation/plugin use first.

### NO-065 — Minimum-version dependency matrix for an exact-pinned SaaS
Tests a contract the product does not promise.

### NO-066 — Replace durable jobs/webhooks with an in-process signal bus
Loses durability/retry/visibility.

### NO-067 — Blind warnings-as-errors flip after years of blanket ignores
Inventory and ratchet first.
