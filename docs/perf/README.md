...
| [`TENANT-GUARD-2026-09-08.md`](TENANT-GUARD-2026-09-08.md) | The dashboard before/after reference R4's AP-inbox pushdown, a cProfile of the request, and the same concurrency run with the ORM tenant guard attaching no options (measurement only) | **PERF-019:** the guard puts ~100 loader-criteria options on every SELECT and SQLAlchemy regenerates their cache key per execute — ~43 % of a dashboard request's loop time, roughly half of every request in the product (serial p95 −40–50 %, throughput ×1.7–2.3 without them). The pushdown itself changed nothing the harness measures (it seeds no approval steps). PERF-018's cause; remedy is its own batch with the Security lens. |
