...
| [`TENANT-GUARD-2026-09-08.md`](TENANT-GUARD-2026-09-08.md) | The dashboard before/after reference R4's AP-inbox pushdown, a cProfile of the request, and the same concurrency run with the ORM tenant guard attaching no options (measurement only) | **PERF-019:** the guard puts ~100 loader-criteria options on every SELECT and SQLAlchemy regenerates their cache key per execute — ~43 % of a dashboard request's loop time, roughly half of every request in the product (serial p95 −40–50 %, throughput ×1.7–2.3 without them). The pushdown itself changed nothing the harness measures (it seeds no approval steps). PERF-018's cause; **remedy shipped the same day (R5): one option per registry, throughput ×1.6–2.1 on every scenario, isolation proofs in the ADR-0004 addendum** — §4. |
| [`SEED-STATISTICS-2026-09-08.md`](SEED-STATISTICS-2026-09-08.md) | The first `--shape` run after the harness began seeding approval steps (reference R6): the dashboard read 8.08× against a 4.0 ceiling | **PERF-020:** the AP-inbox anti join ran as a nested loop over every pending step because the planner estimated one row on never-analysed tables (1.1 M buffer hits, 631 ms); `ANALYZE` alone took the request from 822 to 84 ms. The harness now analyses what it seeds (the steady-state plan is the one to measure); the statement is unchanged. With the inbox's rows the dashboard grows 1.94× — the pushdown's first datapoint of its own. |
