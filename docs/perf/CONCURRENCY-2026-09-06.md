...
| 2026-09-06 | CI `postgres` job, run #553 (P2 batch 4 — the first with the gen-2 threshold at 100) | 300 | 8 | **datapoint 5** — `dashboard` 10.53× contended (conc p95 440 vs p50 245 ms, 27.0 req/s), `ap_aging` 5.79×, `cash_position` 6.70×, `transport_reliability` 6.02×, zero errors; `invoice_create_same_org` serial p95 205 ms (runner noise, 2.20×), across-orgs 11.29× on a 24 ms baseline (the same noise as datapoint 3). Peak RSS 177 MB. |
| 2026-09-06 | CI `postgres` job, run #554 (main, P2 batch 4's deploy run) | 300 | 8 | **datapoint 6** — `dashboard` 11.68× (27.2 req/s), `ap_aging` 5.89×, `cash_position` 6.12×, `transport_reliability` 5.70×, writes 2.36× / 5.45×, zero errors. Six datapoints and the pool measurement agree: one endpoint, CPU-bound on its loop (PERF-018). |
| 2026-09-08 | local (Xeon 2.80 GHz × 4, Postgres 16.13), before/after reference R4's AP-inbox pushdown (PERF-DUCK-001), a worktree at fe66349 against the R4 tree | 300 | 8 | **the pushdown re-measurement (PERF-DUCK-002)** — first pair `dashboard` 9.16× → 10.60× (serial p95 70.5 → 62.6 ms, 17.7 → 16.8 req/s); then THREE alternating pairs (review P-3): the first rewrite read worse on the dashboard in all three (+15–40 ms conc p50) — per-call statement construction, 5.1 vs 3.8 ms — fixed by building the statement once with bound parameters; final medians `dashboard` conc p50 434.5 → 451.1 ms, p95 721.9 → 754.9, 16.1 → 15.1 req/s, all inside the three-run bands; the read itself 2.9 vs 3.8 ms per call. No measurable change at this dataset: the harness seeds no approval steps. `docs/perf/TENANT-GUARD-2026-09-08.md` §1–1a. |
| 2026-09-08 | local, same session, cProfile of 40 serial `/dashboard` requests + the same concurrency run with the ORM tenant guard attaching NO loader options (in-process measurement, RLS still on) | 300 | 8 | **the guard bound — PERF-019 opened, PERF-018's cause named.** ~43 % of the request's loop time is SQLAlchemy cache-key generation over the ~100 `with_loader_criteria` options the guard puts on every SELECT; without them every serial p95 falls 40–50 % and throughput rises 1.7–2.3× on every scenario (`dashboard` 16.8 → 31.4 req/s, `cash_position` 30.8 → 69.4, `invoice_list` 60.9 → 116.2). The dashboard's ratio stays 12.5× — the shape (twenty statements on one loop) is unchanged; the cost halves. `TENANT-GUARD-2026-09-08.md` §2–3. |
| 2026-09-08 | local, reference R5 — the tenant guard as ONE loader-criteria option per registry (PERF-019 remedy), three runs against the R4 final tree's three | 300 | 8 | **the remedy** — throughput ×1.6–2.1 on every scenario (`dashboard` 15.1 → 31.1 req/s, conc p50 451 → 187 ms; `invoice_list` 57.5 → 107.0; `cash_position` 29.7 → 61.2; writes 25.0 → 44.4 / 27.5 → 55.9), zero errors; the dashboard's `x` stays contended (shape). Isolation proofs in ADR-0004's addendum. `TENANT-GUARD-2026-09-08.md` §4. |
| 2026-09-08 | local, reference R6 — the harness seeds one pending approval step per submitted invoice and ANALYZEs after seeding (PERF-020), three runs on the final tree | 300 | 8 | **with the AP inbox's rows present**: `dashboard` 35.1 req/s / conc p50 159.5 ms (R5: 31.1 / 187.2 — inside the run bands, not claimed as a gain; ~40 inbox rows at this scale); `invoice_list` 131.5; `cash_position` 75.3; writes 55.5 / 64.8; zero errors. The `--shape` gate with the same seed: dashboard 1.94× (had read 8.08× on unanalysed tables — `SEED-STATISTICS-2026-09-08.md`). |
