...
| W12 Transport VAT recovery | ✅ vertical | **statement upload (WO-S — the front door: multipart route + intake screen; before it, ingest had no route and no page, so seven parsers and the whole capture gate were reachable only from a Python prompt)** → fuel-card parse (7 networks, detected from the file, never asserted) → gates/tie-out → monthly close → claims frozen at submit → decisions incl. partial (WO-49…85, WO-L) → **the refund landing (WO-T: `approved → paid` with the amount that actually arrived, which is what finally lets median-days-to-refund compute instead of reporting null forever)**; overcharge claim-backs with audited ignore; **WO-Z gave the intake a review QUEUE** — findings persist (`vat_statement_findings`, FORCE RLS), a refused statement now records which line blocked it instead of discarding that with the transaction, and each finding leaves the queue as `resolved` or `dismissed` by a named person |
