...
| Batch | Scope | Status |
|---|---|---|
| **R1 — queue / runtime hardening** | STIR-P1-01 lease heartbeat · STIR-P2-01 OCR runtime budget + `processing_timeout` · PAT-004 Retry-After floor (capped) · PAT-028 connect-time DNS pin (CGNAT/NAT64, SNI, no redirects) · PAT-030 job log context · FLASK-P2-02 route-topology guard | **implemented, reviewed, full regression green; CI on the pushed head → DONE when green (dashboard)** |
| **R2 — billing durability** | BILL-REL-001 durable Stripe webhook (Job row, no new table; customer-mismatch terminal) · BILL-METER-001 frozen usage segment (`reporting_target`, migration `e7f9a1c3d5b8`) · BILL-QA-001 lost-response scenarios | next |
...
| **R4 — performance / product** | PERF-DUCK-001 AP-inbox pushdown (one projected SELECT; no cache, no index) + PERF-DUCK-002 re-measure vs datapoints 5–6 (closes PERF-018) · TW-P2-01 command palette (filtered `navGroups` only; VR baseline reviewed) · FLASK-P3-01 warnings inventory then ratchet | queued |
...
| **Lead Developer** | **APPROVE FOR BRANCH — DONE only when the full regression and CI on the pushed head read green** (recorded in `docs/audit/2026-09-05/DASHBOARD.md`) | |
