...
The complete operator and integrator reference: installation, hosting, deployment sizing, scaling, git setup, reliability, the day-to-day user manual, the external API, SAF-T export, the advisory AI review, and the EU VAT-refund rules reference. Each section below is a previously separate document, preserved in full.
...
- [External API — `/api/v1` (token contract)](#external-api-apiv1-token-contract)
- [Integrator manual — `/api/v1` (external token API)](#integrator-manual-apiv1-external-token-api)
...
> CRM playbook) is [`#integrator-manual-apiv1-external-token-api`](#integrator-manual-apiv1-external-token-api). This page is the short prose
