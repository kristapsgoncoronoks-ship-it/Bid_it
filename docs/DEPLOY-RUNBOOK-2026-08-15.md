...
| `ruff check` / `ruff format --check` | clean (683 files) at the WO-AB tree |
| `mypy app` | clean, 385 files at the WO-AB tree. It earned its keep again: `txn_time` was a VARCHAR in WO-AA, and here it confirmed the new `nace_code`/document paths type through before any test ran |
