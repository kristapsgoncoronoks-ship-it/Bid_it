...
| Alembic | single head `e8f0a2b4c6d8` (onboarding dismissal stamp) — **production is CURRENT** (CI #491 deployed `35f656c`; fresh-Postgres upgrade + `alembic check` verified locally) |
| Browser suite | **384 e2e + 13 visual = 397 passed** on 2026-08-26 at the WO-P tree (4 new onboarding specs), fully clean; `tsc --noEmit` clean. First load 415 kB raw / 122 kB gzip held by `check-bundle`; `check-labels` green |
