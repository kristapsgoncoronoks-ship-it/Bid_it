...
| Browser suite | **401 passed across 38 spec files** (388 e2e + 13 visual — counted with `playwright test --list`, not recalled) on 2026-08-27 at the WO-Q tree, and **carried forward to the WO-R tree unchanged because WO-R touches no frontend file**: `git status --porcelain frontend/` is empty against `578283b`, so the SPA bytes the suite ran against are the bytes here. Independently re-run green by CI #496's `frontend-e2e` job at that same commit. Prior: **397 passed** on 2026-08-26 at the WO-P tree (4 new onboarding specs), fully clean; `tsc --noEmit` clean. First load 415 kB raw / 122 kB gzip held by `check-bundle`; `check-labels` green |
