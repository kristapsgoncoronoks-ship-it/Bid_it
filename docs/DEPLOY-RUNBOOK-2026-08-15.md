...
| `ruff check` / `ruff format --check` | clean (688 files) at the WO-AC tree |
| `mypy app` | clean, 388 files at the WO-AC tree. Prior: 385 at WO-AB. It earned its keep again: `txn_time` was a VARCHAR in WO-AA, and here it confirmed the new `nace_code`/document paths type through before any test ran |
