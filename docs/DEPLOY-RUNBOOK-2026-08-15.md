...
| Check | Result |
|---|---|
| **CI (GitHub runners)** | **run #485: ALL 8 JOBS SUCCESS — including `deploy`**, 2026-08-26 at WO-N `32b42bf` on `main`. The deploy's first attempt timed out (`connect to host … port 22: Connection timed out` — the VPS didn't answer for one window; the same config deployed fine at #483 an hour earlier), and the failed-jobs re-run succeeded in 42s with the 7 code jobs standing green from attempt 1. The new `check-labels` a11y gate ran in this job set. Prior: #483 all-8 (WO-M); #479 all-8 (WO-L, the first CI auto-deploy); #477 7/8 (deploy unreachable, fixed with `ssh -4`); #476/#473/#471/#470/#469/#465 SUCCESS |
| Backend suite | **2804 passed, 11 skipped, 0 failed** — WO-N touches no backend file, so the WO-M certification carries, and CI #485's full pytest (22:38 on the runner) independently re-proved it at the WO-N tree. **Production deployed AT `32b42bf` by CI #485's deploy re-run** |
| `ruff check` / `ruff format --check` | clean |
| `mypy app` | clean, 370 files |
| Alembic | single head `d7e9f1a3b5c7` (bin extension) — **production is CURRENT** (CI #485 deployed `32b42bf`, migrations run by `vps-deploy.sh` on the box) |
| Browser suite | **380 e2e + 13 visual = 393 passed** on 2026-08-26 at the WO-N tree — round 1 had a single vat-claims load flake (the registry mock raced under parallel workers; 3/3 green isolated), round 2 fully clean. `tsc --noEmit` clean; `check-labels` 132 labels associated, proven to bite by seeding an unlabelled Email field |
| Prior certified runs | 2804 / 391 browser 2026-08-26 at WO-M `03e586e` (+ CI #483 all-8-green); 2799 / 390 browser 2026-08-26 at WO-L `23c9b46` (+ CI #479 all-8-green, the first CI auto-deploy); 2791 / 386 browser 2026-08-26 at WO-K `176bbfa` (+ CI #477 code-green); 2782 / 382 browser 2026-08-26 at WO-G2 `edc6c22` (+ CI #473 all-green); 2776 / 380 browser 2026-08-25 at WO-J `7423dca` (+ CI #471 all-green); 2766 / 376 browser 2026-08-25 at WO-I `5d121ee` (+ CI #470 all-green); 2761 / 372 browser 2026-08-25 at the WO-H tree; 2754 / 369 browser 2026-08-25 at `46d3167` (+ CI #465 all-green); 2748 / 366 browser 2026-08-24 at `31e0e0b`; 2745 / 363 browser 2026-08-24 at `3d0f3cb` (one vat-claims flake re-ran clean 45/45; the WO-E round caught a real Customer-card crash, fixed before certifying); 2737 passed 2026-08-23 at `df9642a`; 2729 passed 2026-08-23 at the deployed tree `ee37037`; 2720 at `60e1faf`; 2714 at `2c5e93a`; 2705 2026-08-20 at `d2ba5b0`; 2694 2026-08-16 at `56bcab7` (single environmental failure — tesseract reinstalled, OCR 2/2) |
