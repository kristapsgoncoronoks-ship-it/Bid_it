...
| Worker loop wedged (process alive, loop not turning) | Liveness file older than 180 s (`app.worker_probe`; the loop touches it every tick and every 60 s lease heartbeat) | Kubernetes restarts the pod (5 misses a minute apart); compose shows `unhealthy`; an in-flight job is reclaimed after 300 s. Honest limit: a handler that awaits forever is NOT detected — the lease heartbeat keeps ticking for it and there is no per-job deadline (BE-024). |
