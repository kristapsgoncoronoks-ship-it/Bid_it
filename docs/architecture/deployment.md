...
- **Billing (ADR-0013), opt-in via config.** `BILLING_PROVIDER` = `auto` (default) | `stripe` | `everypay` | `none`; `auto` picks Stripe if its key is set, else EveryPay if its creds are set, else the NullProvider (nothing charges). Common: `BILLING_SUCCESS_URL` (must end in `?checkout=success` — the SPA keys its post-Checkout activating state on it), `BILLING_CANCEL_URL`, `API_PUBLIC_BASE_URL` (used to build the provider return/callback URLs).
  - **Stripe** (subscription): `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET` (the webhook fails **closed** without it), per-plan `STRIPE_PRICE_STARTER`/`STRIPE_PRICE_PRO`, `BILLING_PORTAL_RETURN_URL`. Point the dashboard webhook at `POST /api/v1/billing/webhook` for `checkout.session.completed` + `customer.subscription.{updated,deleted}`. `pip install stripe`. Seller-of-record: enable **Stripe Tax** for VAT calc; remittance/filing stays a finance task. For **metered/overage** pricing, create a Stripe Billing Meter and set `STRIPE_METER_UPLOAD` to its `event_name`; the worker's daily `billing.report_usage` job then reports frozen, retry-safe usage segments (the worker tier must be running — it is also what applies Stripe webhook events, which the API only persists as jobs).
  - **EveryPay** (Baltic cards, redirect): `EVERYPAY_API_USERNAME`, `EVERYPAY_API_SECRET`, `EVERYPAY_ACCOUNT_NAME`, `EVERYPAY_API_BASE_URL` (test `https://igw-demo.every-pay.com/api/v4`, live `https://pay.every-pay.eu/api/v4`). Configure the processing account's `customer_url`/`callback_url` to reach `…/billing/everypay/return` and `…/billing/everypay/callback` (derived from `API_PUBLIC_BASE_URL`). Recurring MIT charges run on the **worker** (`everypay.charge_mit`) — the worker tier must be running for renewals to bill.
- **SSO (OIDC, ADR-0021):** per-tenant connection config lives in `sso_connections` (admin UI), not env. Set `API_PUBLIC_BASE_URL` so the OIDC redirect URI (`…/api/v1/auth/sso/callback`) is correct, and `SSO_POST_LOGIN_URL` / `SSO_ERROR_URL` to the SPA callback/login pages. Register the redirect URI with each tenant's IdP. **Before GA:** move `sso_connections.client_secret` to the envelope-encrypted secret store, and run the live-IdP/Keycloak smoke (the ADR-0021 "return to finish" seams).
- **Data residency (ADR-0022):** set `SERVICE_REGION` per deployment and `DEFAULT_TENANT_REGION` for new tenants; turn on `ENFORCE_REGION_PINNING` in a multi-region setup so a tenant pinned elsewhere is refused **421** (backstop behind the LB's region routing). Real residency also requires region-local Postgres + object storage (`STORAGE_S3_BUCKET`) + backups and per-region LB routing — infra, not app config.
...
- **Metrics** via Prometheus (`/metrics`, gated by `METRICS_ENABLED`): request rate/latency, DB pool, webhook delivery success, parse method mix, per-tenant usage, and **queue health** — `invoiceiq_jobs{status}` (counts by state incl. dead-letter) + `invoiceiq_jobs_dead{kind}` (dead-letter depth by job kind, the alert-routing key; a kind that recovers is re-published as 0) + `invoiceiq_jobs_oldest_pending_seconds` (queue-lag SLO), refreshed by the worker each loop.
- **Health probes:** `/health` (liveness), `/health/ready` (readiness: DB `SELECT 1`), **`/health/queue`** (background-queue SLO — aggregate counts + dead-letter depth, also by kind (`dead_by_kind`) + oldest-pending age; **503 when degraded** so an uptime check pages; thresholds `QUEUE_SLO_MAX_PENDING_AGE_SECONDS` / `QUEUE_DLQ_ALERT_THRESHOLD`). The **worker** has no HTTP port: its liveness is a heartbeat file touched every loop tick and every lease heartbeat, checked by `python -m app.worker_probe` (compose `healthcheck`, k8s `livenessProbe`; `WORKER_LIVENESS_PATH` / `WORKER_LIVENESS_MAX_AGE_SECONDS`) — a wedged loop fails it, a queue backlog deliberately does not.
