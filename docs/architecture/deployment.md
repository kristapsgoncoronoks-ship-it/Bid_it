...
- **Billing (ADR-0013), opt-in via config.** `BILLING_PROVIDER` = `auto` (default) | `stripe` | `everypay` | `none`; `auto` picks Stripe if its key is set, else EveryPay if its creds are set, else the NullProvider (nothing charges). Common: `BILLING_SUCCESS_URL`, `BILLING_CANCEL_URL`, `API_PUBLIC_BASE_URL` (used to build the provider return/callback URLs).
