...
13. 🟡 **Audit-log export** shipped: owner-gated `GET /audit/export?fmt=csv|json` (+ action/date filters) streams the hash-chained trail **including the seq/prev_hash/hash columns** so an auditor can re-verify the chain offline; formula-injection-safe CSV; chain-verify status in the JSON body + response headers. SOC 2 / ISO evidence. Still open in Phase 4: SSO/SAML/SCIM (needs an IdP to prove), region-pinning, SOC 2/ISO controls, GDPR erasure (DSAR).
