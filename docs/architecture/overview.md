...
9. 🟡 **Billing — Stripe** wired to the existing plan/entitlement model (ADR-0013): a provider seam (NullProvider default → nothing charges; StripeProvider when a key is set), Stripe **Checkout** + **Customer Portal**, and a **signed webhook as the authority** for plan/status applied **idempotently** (`processed_stripe_events`). We are seller-of-record → EU VAT is our responsibility (Stripe Tax calculates; remit/file is a documented follow-up). Deferred: usage-overage metering to Stripe + Stripe Tax enablement.
