...
9. 🟡 **Billing — Stripe + EveryPay** wired to the existing plan/entitlement model (ADR-0013): one provider seam, `billing_provider`-selected. **Stripe** (subscription) = Checkout + Portal + signed webhook as the authority. **EveryPay** (Baltic card gateway, redirect kind) = hosted payment page → **server-side verify** (never the redirect) via `billing_payments`, tokenised initial payment, and **merchant-initiated recurring** on the job queue (`everypay.charge_mit`). Both apply plan/status **idempotently**. We are seller-of-record → EU VAT is our responsibility (Stripe Tax calculates; remit/file is a documented follow-up). **Metered-usage overage** reports to Stripe as frozen, retry-safe segments (Billing Meter events whose identifier derives from the committed `reporting_target`, `billing.report_usage` daily job); the Stripe webhook itself only persists the event as a job — the worker applies it. Deferred: Stripe Tax enablement + EveryPay advanced dunning.
...
18. 🟡 **Queue-health observability** shipped: `queue_health.snapshot` (cross-tenant) surfaces dead-letter depth + oldest ready-but-unprocessed age (queue-lag SLO); a **`/health/queue`** probe returns **503 when degraded** (DLQ over threshold or lag past `queue_slo_max_pending_age_seconds`) so an uptime check pages, and Prometheus gauges (`invoiceiq_jobs{status}`, `invoiceiq_jobs_dead{kind}`, `invoiceiq_jobs_oldest_pending_seconds`) are refreshed by the worker each loop; the worker's own liveness is a heartbeat file probed by `app.worker_probe` (compose healthcheck / k8s livenessProbe), separate from the queue SLO by design. Still open in Phase 4: SOC 2/ISO controls; fat-tenant dashboard latency + distributed rate limiting remain scale-gated.
19. 🟡 **Data residency / region-pinning** shipped (ADR-0022): `organizations.region` (assigned at registration), a `service_region` per deployment, and an **enforcement backstop** — a tenant pinned to a different region than the deployment is refused **421** at `get_current_user` + login (`core/residency`), off by default so single-region stays byte-identical. **Boundary:** the app seam only — real residency needs region-local DB/object-storage/backups + LB routing per region (deployment), and tenant relocation isn't modelled.
...
│   │   ├── worker_probe.py        # Worker liveness probe (container healthcheck / livenessProbe)
