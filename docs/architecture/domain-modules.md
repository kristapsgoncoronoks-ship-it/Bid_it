...
| **Jobs & Scheduler** | `jobs` | `jobs`, `scheduler`, `job_handlers`, `worker`, `worker_probe`, `queue_health` | all (as handlers) | Durable queue; handlers run in tenant scope; `/health/queue` SLO probe. |
...
| **Capture-review learning loop** | `capture_field_memory` | `capture_memory` | extraction fields (via routes) | E1.5: per-(vendor × field) correction history keyed on the raw captured header `vendor_name`; `hints_for` is an ADVISORY, read-only suggestion attached to `GET/POST .../captures/{id}/fields|review` — never auto-applied to `ExtractionField.value`/`.reviewed_value` (§4.19). |
| **Invoices (AP)** | `invoices`, `line_items` | invoice routes, `validation` | vendors, fx, vat, dimensions | The core received-invoice record. Duplicate detection (`duplicates`, `GET /invoices/duplicate-candidates`) is advisory, never blocks: exact-number matches (`exact`/`cross_supplier`) plus (E1.4) `scored` — same-vendor, close amount + issue date, DIFFERENT number. |
