...
- **Slice 3+ (next):** `currencies`/`tax_codes` reference tables; `payment_allocations`; `documents`/`document_versions`/`extraction_runs`/`extraction_fields`; approvals; polymorphic attachments/comments/notifications.
