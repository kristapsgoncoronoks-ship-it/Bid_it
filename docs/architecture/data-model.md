...
- **Slice 3a (shipped):** write-path link resolution for invoices — `costing.resolve_link_id`/`apply_links` link a cost-allocation tag to its master row at invoice **create/update** (not only on the backfill sweep). Re-tagging follows the link; clearing or an unmatched tag nulls it. (Expense-item write-path resolution — the same helper on the expense create/update sites — is the small follow-up; the backfill still covers them meanwhile.)
- **Slice 3+ (next):** `currencies`/`tax_codes` reference tables; `payments`/`payment_allocations` extracted from inline AR fields; `documents`/`document_versions`/`extraction_runs`/`extraction_fields`; approvals; polymorphic attachments/comments/notifications.
