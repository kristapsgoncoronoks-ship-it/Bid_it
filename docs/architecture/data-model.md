...
- **Slice 5+ (next):** `payment_allocations` (one receipt across many invoices); `extraction_fields` (per-field capture confidence); `document_versions`; approvals; polymorphic attachments/comments/notifications.
