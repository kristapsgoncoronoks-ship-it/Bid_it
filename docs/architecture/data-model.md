...
- **Slice 4+ (next):** a `currencies` reference table; `payment_allocations`; `documents`/`document_versions`/`extraction_runs`/`extraction_fields`; approvals; polymorphic attachments/comments/notifications.
