...
- **Slice 4+ (next):** link issued-invoice lines to `tax_codes` (`tax_code_id`, resolve at issue); a `currencies` reference table; `payment_allocations`; `documents`/`document_versions`/`extraction_runs`/`extraction_fields`; approvals; polymorphic attachments/comments/notifications.
