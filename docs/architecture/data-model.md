...
**`invoices`** (supplier invoices) — money stored as three separate quantities per the tax-total rule: `subtotal` (tax-exclusive), `tax_amount`, `total` (tax-inclusive), all `Numeric(14,2)`; original currency (`currency`) **and** reporting currency (`total_eur` + `fx_rate` + `fx_source` provenance). **`fx_source` is a closed enum (WO-8)** — `{eur, stated, ecb, unknown}` (`models/fx.FxSource`), CHECK-constrained (`ck_invoices_fx_source`, same on `expense_items`); rates follow the single ECB convention (units per 1 EUR, converting to EUR divides) and `unknown ⇒ total_eur IS NULL`, never a guessed figure. `issue_date` indexed with `org_id`. **`po_reference`** (`d3e4f5a6b7c8`, backlog N1) carries EN-16931 **BT-13**, the supplier's purchase-order reference: `VARCHAR(60)`, nullable, free text (it is the buyer's own identifier, so no format may be assumed) and deliberately the SAME width as `issued_invoices.po_reference`, so a reference round-trips through either side of the product. Captured from UBL `cac:OrderReference/cbc:ID` and CII `BuyerOrderReferencedDocument/IssuerAssignedID`; absent stays NULL — never inferred from the invoice's own id. **Target:** `cost_center_id`/`department_id`/`project_id` FKs (Slice 2).
...
- **Slice 5f (`98d1896eeb26`, shipped):** **`extraction_fields`** — per-field capture provenance, deepening the Slice 5b run into one row per top-level field. The prerequisite shipped with it: `parser._provenance` now emits, for each of the five header fields (invoice_number, vendor_name, issue_date, due_date, currency — **`po_reference` joined them with N1, `d3e4f5a6b7c8`; `extraction_provider._HEADER_FIELDS` is the live set**), whether it was `extracted` from the source / `defaulted` / `missing`, carried on `ParsedInvoiceDraft.fields` and surfaced on the upload response. `extraction.record_fields` persists them against the run at upload; `GET /invoices/{id}/extraction` returns each run with its `fields`. A `confidence` column (`Numeric(4,3)`, nullable) is reserved for the OCR/AI path — the deterministic CSV/JSON parsers leave it null. Composite FK `(org_id, extraction_run_id) → extraction_runs(org_id, id)` (runs gained `UNIQUE(org_id, id)`), `ON DELETE CASCADE`; RLS + ORM guard.
