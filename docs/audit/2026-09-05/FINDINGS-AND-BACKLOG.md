...
| QA-011 (new, 2026-09-06) | The SQLite suite runs with foreign-key enforcement OFF: `app.core.database` sets `PRAGMA foreign_keys=ON` on the app engine's connections, but `tests/conftest.py` builds its own in-memory engine without that listener, so no test has ever exercised an `ON DELETE` rule or an FK refusal — the batch-3 RESTRICT tests passed vacuously until they switched the pragma on for their own connection | MEDIUM | CONFIRMED (three tests "DID NOT RAISE" with the constraint in place; raised once the pragma was set) | ACCEPT — P2: mirror the listener in the test engine and run the suite; whatever it breaks is a real orphaning path |
...
| P2 set (remaining) | P2 | DB-014, ARCH-002/008, PERF-018 (the dashboard's per-request CPU — CONC-001's remedy), OPS-003/007/009/011/013, QA-005/006/007/011, FE-011/013/018/019/021 (FE-004's ratchet still lists 20 pages), PROD-005/009 (PROD-009's ex-client export half = WO-AI DONE; workspace deletion / whole-tenant export still open) | various | | | | | IN PROGRESS by batches — batches 3 (data integrity + correctness) and 4 (performance) delivered; what remains is the QA, frontend, ops/config groups, DB-014 and PERF-018 |
