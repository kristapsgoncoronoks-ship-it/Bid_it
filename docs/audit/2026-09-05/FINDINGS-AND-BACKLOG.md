...
| R2-B2 | P2 | FE-001 backstop double-surfaces on 6 pages that render `mutation.error` inline without `onError` (Billing, Issuer, Sessions, VerifyEmail, ResetPassword, Portal) | Frontend | Phase 12 review | LOW | S | e2e: one surface | NOT STARTED |
| R2-A3 | P2 | 43 page-level controls still gate on the stored-role mirror (`hasVatPerm`/`isAdminOrAbove`) while the nav reads served permissions (= PROD-010 scope) | Frontend | Phase 12 review | MED | M | e2e per role | NOT STARTED |
| R2-T2 | P2 | Threadpool test covers `ensure_customer` only; `start_checkout`, `create_portal_url`, `report_usage` unasserted | QA | Phase 12 review | LOW | S | test | NOT STARTED |
