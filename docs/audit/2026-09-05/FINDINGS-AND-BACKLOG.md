...
| CONC-001 (new, 2026-09-06) | First concurrency datapoint (CI #543, 8 in flight, scale 300): `dashboard` 12.5×, `ap_aging` 10.1×, `cash_position` 10.4× the serial p95 — more than serialising would cost; `invoice_list`/`explore`/`reliability` 2–5× (overlapping, as expected). First suspect: the connection pool (default 5 + 10 overflow) under multi-statement aggregate reads; second: shared-row locks. One datapoint on a shared runner | MEDIUM | LIKELY (single datapoint; harness artefacts removed for the next) | ACCEPT — P2: measure pool size as a variable in the next datapoints before any code moves |
