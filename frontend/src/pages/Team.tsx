        ...
        <div className="card space-y-3">
          ...
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div>
              ...
              <select className="input w-32" value={role} onChange={(e) => setRole(e.target.value as "member" | "owner")}>
            ...
            <div className="space-y-2 border-t border-slate-100 pt-3">
              ...
              {invites.data!.map((iv) => (
                <div key={iv.id} className="flex flex-wrap items-center gap-2 rounded-lg bg-slate-50 px-3 py-2 text-sm">
                  ...
                  <span className="badge bg-slate-200 text-slate-600">{iv.role}</span>
              ...
              <tr key={m.id} className="border-b border-slate-100 last:border-0">
                ...
                <td className="px-4 py-3">
                  {isOwner && m.id !== user?.id ? (
                    <select className="input w-28 py-1 text-xs" value={m.role} onChange={(e) => patch.mutate({ id: m.id, body: { role: e.target.value as "member" | "owner" } })}>
                      <option value="member">member</option>
                      <option value="owner">owner</option>
                    </select>
                  ) : (
