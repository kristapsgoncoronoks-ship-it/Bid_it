...
| **AI capture & verify** *(opt‑in, default‑OFF, advisory)* | For scanned / messy / unknown‑layout PDFs an opt‑in **vision capture** reads the page images with a vision model into a structured "capture document" that flows into the **same human review + confirm gate** (never authoritative); an **independent verify model** checks the captured draft field‑by‑field against the PDF (the PDF is the source of truth), can apply PDF‑authoritative corrections and re‑verify, and the captured data is persisted as a permanent **JSON+text second file** beside the original. A per‑(supplier × field) **learning loop** flags the fields a supplier tends to mis‑read, and a **classification / DLP overlay** labels each document's sensitivity and (opt‑in) can block over‑sensitive documents from any external AI. All advisory — it never mutates a figure without the human confirm. |
...
| **Reports & export** | **Expense / cost reports** (`/expenses` + Excel), a supplier **price‑review packet** (avoidable‑overpay), an **accounting‑ledger CSV**, a programmable **SAF‑T (OECD core) XML** export, and **EN‑16931 / UBL 2.1 e‑invoice XML** export of registered invoices (single + batch ZIP) — all gathered in an **Accounting & ERP exports** hub, NET EUR basis, totals reconciled to the same ledger math. |
| **Document management** | A full DMS overlay over the vault: **full‑text search** (FTS5), **typed custom fields + hierarchical tags**, append‑only **version chains**, and **retention policies + legal hold** (advisory records‑management — flags records past retention for human review, never auto‑deletes). |
| **Secure sharing & e‑sign** | Trackable public **share links** (`/s/<token>`) over a vaulted document with expiry / password / require‑email gates, an **NDA gate + dynamic watermark**, page‑by‑page engagement analytics, branded **data rooms**, and **e‑signature (SES)** with a SHA‑256 integrity binding + verify. |
| **Workflow** | A configurable, advisory **approval / routing engine** — ordered steps (approve / sign / notify / tag) with a Tasks/Approvals inbox; additive process tracking that **never overrides the VAT legal gates**. |
| **AI agent access (MCP)** | An optional **read‑only, token‑gated, tenant‑aware MCP server** that exposes platform data (search, reclaimable VAT, claim status, benchmark, KPIs, document metadata, overdue requests) to AI agents — never bank/secret data; the `mcp` SDK is an optional extra (`requirements-mcp.txt`). See **[docs/MCP.md](docs/MCP.md)**. |
...
| **Embedded finance & bank recon** *(advisory)* | A financeable‑receivable view with a **per‑claim advance / fee origination model** (advance, net‑now/net‑later, expected payout) and an **advances ledger** on the Receivables page (origination‑only seam; a licensed factoring partner plugs in behind a NullProvider default), and **open‑banking reconciliation** matching uploaded bank credits against expected VAT refunds (`/recon`). Both are advisory — they never move money or mutate a VAT figure/lock. |
...
| **[docs/MCP.md](docs/MCP.md)** | The read‑only MCP server — the tools it exposes to AI agents, transports (stdio / streamable‑HTTP), the token gate, and the optional `mcp` SDK install. |
...
├── vision_capture.py ai_verify.py  # OPT-IN advisory: vision capture of a PDF → draft; verify-vs-PDF + apply corrections (independent model)
├── capture_file.py capture_confidence.py classify.py  # persist capture as 2nd file; per-supplier×field accuracy learning; sensitivity/DLP gate
├── ai_assistant.py            # OPT-IN advisory chat over a document's DERIVED data only
...
├── search.py metadata.py versioning.py retention.py  # DMS overlays: FTS5 search, typed fields+tags, version chains, retention+legal hold
├── sharing.py share_watermark.py esign.py  # secure share links, NDA/watermark/page-analytics/data-rooms, e-signature (SES)
├── mcp_tools.py mcp_server.py        # read-only, token-gated, tenant-aware MCP server (optional `mcp` SDK; see docs/MCP.md)
