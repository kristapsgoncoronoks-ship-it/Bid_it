...
- 🧳 **Employee expenses** — employees submit receipted expense reports (per-employee ownership within the tenant); managers approve/reject/reimburse. Reclaimable VAT + FX-to-EUR (ECB), receipt attachments, and a PDF export. **Import a bank statement** (PDF via OCR, or CSV) to auto-fill expenses — a separate reader built on the same engine that tells the transaction amount from the running balance.
