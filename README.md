...
**Scale of the codebase (verified against this tree):** 104 database tables
(113 Alembic revisions, single head), 60 model modules, 99 service modules,
46 route modules, 66 SPA pages, 2787 collected backend tests, 8 CI jobs.
...
- **Tenant isolation, three layers, tested**: per-query `org_id` filters + an ORM
  `do_orm_execute` guard over a 97-model registry + Postgres `FORCE ROW LEVEL
