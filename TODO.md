...
| **M2** | "We can take money" — billing go-live | 🔶 **In Progress** — WO-47 (quota model) + WO-48 (dogfood billing fallback) + **WO-AD** (2026-09-05: retention rides the ladder — Business/Enterprise 7y, one shared extend-only re-stamp on every plan-change path; `STRIPE_PRICE_BUSINESS` + `purchasable` so the SPA never offers a checkout that can only 502; `STRIPE_AUTOMATIC_TAX` flag; cap policy = block, over-grant = left intentional) shipped. Owner-side only remains: live credentials incl. the Business price id, the Billing Meter `event_name`, seller-of-record VAT filing. |
