...
- [x] **WO-82** — `Completed` — G4.5/R41: supplier overcharge detection + the claim-back
  ...
  *"two term types only"* (`expected_discount_eur_l`, a rebate in €/L; `max_net_eur_l`, a
  ...
  created them — never EXEMPT. 81 tables, 85 revisions. **DEFERRED, not skipped:** R41's two
  send-ready ARTIFACTS (the Excel evidence packet and the formal PDF claim letter with a 30-day
  credit/refund demand, *"built from the SAME line source"*) are a named follow-up slice of G4.5
  — `contract_audit.audit()` already IS that single line source, which is what will make the
  *"identical lines and totals"* acceptance structural rather than coincidental (the G2.12
