...
async def finish_login(db: AsyncSession, connection: SsoConnection, *, code: str,
    ...
    user, org = await _match_or_provision(db, connection, email=email, name=name)
    await audit.record(db, "sso.login", org_id=org.id, actor=(user.id, user.email),
                       target_type="user", target_id=user.id, meta={"provider": connection.slug})
    await db.commit()
    return user, org


async def _match_or_provision(db: AsyncSession, connection: SsoConnection, *, email: str,
                              name: str) -> tuple[User, Organization]:
    ...
    role = connection.default_role if connection.default_role in UserRole.__members__ \
