...
_TRANSITIONS = {
    ...
    "project": {"active": {"closed", "archived"}, "closed": {"active", "archived"},
                "archived": {"active"}},
}
...
def _kind(model) -> str:
    return {"departments": "department", "cost_centers": "cost_center",
            "projects": "project"}[model.__tablename__]
...
async def create_cost_center(db: AsyncSession, org_id: str, *, code: str, name: str,
                             department_id: str | None = None) -> CostCenter:
    ...
    if department_id is not None:
        ...
        dep = await db.scalar(select(Department).where(
            Department.org_id == org_id, Department.id == department_id))
        if dep is None:
...
async def create_project(db: AsyncSession, org_id: str, *, code: str, name: str,
                         start_date=None, end_date=None) -> Project:
...
async def list_entities(db: AsyncSession, model, org_id: str, *, include_archived: bool = False) -> list:
...
async def rename(db: AsyncSession, model, org_id: str, entity_id: str, *, name: str,
                 expected_version: int) -> object:
...
async def set_status(db: AsyncSession, model, org_id: str, entity_id: str, *, status: str,
                     expected_version: int) -> object:
    row = await _load(db, model, org_id, entity_id)
    _check_version(row, expected_version)
