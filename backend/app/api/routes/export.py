...
async def list_formats(current: CurrentUser):
    ...
    return [{"key": f, "label": {"generic": "Generic accounting CSV",
                                 "xero": "Xero (Bills)",
                                 "quickbooks": "QuickBooks (Bills)"}[f]}
...
async def export_accounting(
    ...
    if not is_admin_or_above(current):
        raise HTTPException(status.HTTP_403_FORBIDDEN, "Only an admin can export the accounting ledger")
