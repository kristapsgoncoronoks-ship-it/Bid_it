...
async def _import(client, token):
    ...
    return (await client.post("/api/v1/expenses/import/bank-statement", files=files, headers=_h(token))).json()
...
async def test_report_from_inbox_transactions(auth_client, client):
    ...
    r = await client.post("/api/v1/expenses", json={"title": "May card", "transaction_ids": ids}, headers=_h(emp))
    ...
    # ...and can't be reused.
    dup = await client.post("/api/v1/expenses", json={"title": "again", "transaction_ids": ids}, headers=_h(emp))
...
async def test_add_transaction_to_existing_draft(auth_client, client):
    ...
    rid = (await client.post("/api/v1/expenses", json={"title": "Trip"}, headers=_h(emp))).json()["id"]
    r = await client.post(f"/api/v1/expenses/{rid}/items/from-transaction",
                          json={"transaction_id": tid, "category": "software"}, headers=_h(emp))
...
async def test_item_comment_and_receipt(auth_client, client):
    ...
    r = await client.post("/api/v1/expenses", json={"title": "Trip", "items": [
        {"spend_date": "2026-05-01", "category": "meals", "description": "Dinner",
         "amount": "42.00", "vat_amount": "0", "comment": "Client dinner, 3 people"}]}, headers=_h(emp))
    rep = r.json()
    item = rep["items"][0]
    ...
    up = await client.post(f"/api/v1/expenses/{rep['id']}/items/{item['id']}/receipt", files=files, headers=_h(emp))
...
async def test_comment_thread_between_employee_and_manager(auth_client, client):
    ...
    c2 = await auth_client.post(f"/api/v1/expenses/{rid}/comments", json={"body": "Approved — thanks"})
    ...
    assert (await client.get(f"/api/v1/expenses/{rid}/comments", headers=_h(other))).status_code == 404
