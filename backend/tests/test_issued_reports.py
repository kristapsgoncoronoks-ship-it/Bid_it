...
def _inv(buyer, number_price, *, issue, due, scheme="standard", rate="21"):
    return {
        "buyer_name": buyer, "buyer_vat_number": f"VAT-{buyer[:3].upper()}",
        "issue_date": issue, "due_date": due, "vat_scheme": scheme,
        "lines": [{"description": "Service", "quantity": "1", "unit_price": number_price, "vat_rate": rate}],
    }


async def _seed(auth_client):
    # Acme: two invoices (one paid, one overdue). Globex: one open (future due).
    a1 = (await auth_client.post("/api/v1/issued", json=_inv("Acme", "1000.00", issue="2026-01-10", due="2026-02-10"))).json()
    a2 = (await auth_client.post("/api/v1/issued", json=_inv("Acme", "500.00", issue="2026-02-01", due="2026-03-03"))).json()
    g1 = (await auth_client.post("/api/v1/issued", json=_inv("Globex", "200.00", issue="2026-03-05", due="2099-01-01"))).json()
    return a1, a2, g1
...
async def test_payment_recording_and_derived_status(auth_client):
    ...
    paid = await auth_client.patch(f"/api/v1/issued/{a1['id']}/payment",
    ...
    bad = await auth_client.patch(f"/api/v1/issued/{a2['id']}/payment", json={"amount_paid": "999999"})
    ...
    part = await auth_client.patch(f"/api/v1/issued/{g1['id']}/payment", json={"amount_paid": "100.00"})
...
async def test_receivables_paid_unpaid_overdue_and_aging(auth_client):
    await _activate(auth_client)
    a1, a2, g1 = await _seed(auth_client)
    await auth_client.patch(f"/api/v1/issued/{a1['id']}/payment",
                            json={"amount_paid": "1210.00", "paid_date": "2026-02-05"})
...
async def test_vat_summary_by_rate_and_scheme(auth_client):
    ...
    await auth_client.post("/api/v1/issued", json=_inv("Acme", "1000.00", issue="2026-01-10", due="2026-02-10"))
    await auth_client.post("/api/v1/issued", json=_inv("Zeta", "400.00", issue="2026-01-11", due="2026-02-11",
