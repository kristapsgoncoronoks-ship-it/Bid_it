...
def _invoice(partner_id, price="1000.00", *, issue, due, penalty=None):
    body = {
        ...
        "buyer_name": "ignored-when-partner",
        "issue_date": issue,
        "due_date": due,
        "lines": [
...
async def test_workflow_gate_blocks_until_signed(auth_client):
    ...
    await auth_client.post(
        f"/api/v1/partners/{p['id']}/documents/{c['id']}/sign", json={"signed_by": "CEO"}
    )
    r = await auth_client.post(
        "/api/v1/issued", json=_invoice(p["id"], issue="2026-06-01", due="2026-06-30")
    )
    ...
    a = (
        await auth_client.post(
            f"/api/v1/partners/{p['id']}/documents",
            json={"kind": "acceptance_act", "title": "Acceptance act #1"},
        )
    ).json()
    signed = (
        await auth_client.post(f"/api/v1/partners/{p['id']}/documents/{a['id']}/sign", json={})
    ).json()
...
async def test_penalty_invoice_requires_contract_then_generates(auth_client):
    ...
    c = (
        await auth_client.post(
            f"/api/v1/partners/{p['id']}/documents", json={"kind": "contract", "title": "Contract"}
        )
    ).json()
    ...
    await auth_client.post(
        "/api/v1/issued", json=_invoice(p["id"], "1000.00", issue="2025-12-01", due="2026-01-01")
    )
    await auth_client.post(
        "/api/v1/issued", json=_invoice(p["id"], "500.00", issue="2025-12-01", due="2026-01-01")
    )
...
async def test_penalty_invoice_blocked_when_disabled(auth_client):
    await _activate(auth_client)
    p = await _partner(
        auth_client, requires_contract=True, penalty_enabled=False, penalty_rate="12"
    )
    c = (
        await auth_client.post(
            f"/api/v1/partners/{p['id']}/documents", json={"kind": "contract", "title": "C"}
        )
    ).json()
    await auth_client.post(f"/api/v1/partners/{p['id']}/documents/{c['id']}/sign", json={})
    await auth_client.post(
