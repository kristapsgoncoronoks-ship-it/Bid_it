...
async def _emit(auth_client, n=3):
    ...
    for i in range(n):
        r = await auth_client.post("/api/v1/invoices", json={
            ...
            "line_items": [{"description": "Item", "quantity": "1", "unit_price": "100", "tax_rate": "0"}],
        })
...
def test_csv_injection_safe():
    ...
    e = AuditEvent(org_id="o", seq=1, action="x", at_ms=0, hash="h",
                   actor_email="=cmd()", meta='{"k":"v"}', prev_hash=None)
...
def test_json_includes_chain_and_events():
    ...
    e = AuditEvent(org_id="o", seq=1, action="invoice.create", at_ms=1_700_000_000_000,
                   hash="abc", actor_email="a@a.io", meta='{"n":1}', prev_hash=None,
                   target_type="invoice", target_id="42")
    doc = json.loads(audit_export.to_json([e], ChainStatus(True, 1), org_id="o", generated_ms=1_700_000_000_000))
