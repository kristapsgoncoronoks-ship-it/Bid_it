...
async def _seed_subject(db, org_id, *, with_attachment=True):
    ...
    sha = None
    if with_attachment:
        sha, _ = await documents.store(documents.EMAIL_ATTACHMENTS, org_id, b"attach", "application/pdf")
    db.add(InboundInvoice(org_id=org_id, received_at=datetime.now(timezone.utc),
                          filename="a.pdf", status="pending", from_addr=SUBJECT, sha256=sha))
    db.add(IssuedInvoice(org_id=org_id, number="INV-1", issue_date=date(2026, 1, 1),
                         buyer_name="Jane Doe", buyer_email=SUBJECT, seller_json="{}"))
    await db.commit()
    return u, sha
...
async def test_erase_is_audited_with_hashed_subject(auth_client, db_session):
    ...
    ev = await db_session.scalar(
        select(AuditEvent).where(AuditEvent.org_id == org_id, AuditEvent.action == "privacy.erasure")
    )
