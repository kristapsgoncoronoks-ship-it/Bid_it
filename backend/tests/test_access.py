...
async def test_only_platform_operator_edits_the_global_matrix(auth_client, client, db_session):
    ...
    assert (await client.put("/api/v1/access/matrix/user_free",
            json={"monthly_invoice_limit": 5, "monthly_upload_limit": 5}, headers=_h(u))).status_code == 403

    # NEITHER can a company OWNER — the matrix is global, not their company's.
    denied = await auth_client.put("/api/v1/access/matrix/user_free",
...
async def test_admin_cannot_manage_users_but_owner_can(auth_client, client):
    ...
    assert (await auth_client.post("/api/v1/team/invites", json={"email": "y@acme.io", "role": "user"})).status_code == 201
