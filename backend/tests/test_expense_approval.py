...
async def _member(auth_client, client, email, name="Employee"):
    ...
    acc = await client.post("/api/v1/auth/accept-invite",
...
async def _complete_and_submit(client, rid, headers):
    ...
    for it in rep["items"]:
        await client.patch(f"/api/v1/expenses/{rid}/items/{it['id']}", json={"comment": "Client visit"}, headers=headers)
        await client.post(f"/api/v1/expenses/{rid}/items/{it['id']}/receipt",
                          files={"file": ("r.png", _PNG, "image/png")}, headers=headers)
    return await client.post(f"/api/v1/expenses/{rid}/submit", headers=headers)


def _payload():
    return {"title": "Trip", "items": [
...
async def test_owner_is_approver_by_default_and_appoints_others(auth_client, client):
    ...
    assert (await client.post(f"/api/v1/expenses/{rid}/decision", json={"action": "approve"}, headers=_h(tok))).status_code == 403
    ...
    ok = await client.post(f"/api/v1/expenses/{rid}/decision", json={"action": "approve"}, headers=_h(tok))
...
async def test_cannot_approve_own_report(auth_client, client):
    ...
    assert (await client.post(f"/api/v1/expenses/{rid}/decision", json={"action": "approve"}, headers=_h(tok))).status_code == 200
...
async def test_cannot_remove_last_approver(auth_client, client):
    ...
    r = await auth_client.patch(f"/api/v1/team/members/{me['user']['id']}", json={"is_expense_approver": False})
...
async def test_bank_statement_verification(auth_client, client):
    ...
    imp = await client.post("/api/v1/expenses/import/bank-statement",
                            files={"file": ("stmt.csv", io.BytesIO(csv), "text/csv")}, headers=_h(emp_tok))
    ...
    cands = (await client.get(f"/api/v1/expenses/{rid}/items/{item['id']}/match-candidates", headers=_h(emp_tok))).json()
    assert len(cands) == 1 and cands[0]["amount"] == "42.00"

    # …and matching it verifies the item.
    m = await client.post(f"/api/v1/expenses/{rid}/items/{item['id']}/match",
                          json={"transaction_id": cands[0]["id"]}, headers=_h(emp_tok))
    ...
    um = await client.delete(f"/api/v1/expenses/{rid}/items/{item['id']}/match", headers=_h(emp_tok))
...
async def test_match_rejects_amount_mismatch(auth_client, client):
    ...
    await client.post("/api/v1/expenses/import/bank-statement",
                      files={"file": ("s.csv", io.BytesIO(csv), "text/csv")}, headers=_h(emp_tok))
    rid = (await client.post("/api/v1/expenses", json=_payload(), headers=_h(emp_tok))).json()["id"]
    ...
    bad = await client.post(f"/api/v1/expenses/{rid}/items/{item['id']}/match",
                            json={"transaction_id": txn["id"]}, headers=_h(emp_tok))
