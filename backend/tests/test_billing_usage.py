...
class FakeStripe:
    ...
    async def report_usage(self, *, customer_id, meter_event, quantity, identifier):
        self.calls.append({"customer_id": customer_id, "meter_event": meter_event,
...
async def _seed_usage(db, org_id, count, reported=0):
    db.add(UsageCounter(org_id=org_id, period=_period(), metric="upload",
                        count=count, reported=reported))
    await db.commit()
...
async def test_null_provider_report_usage_raises():
    with pytest.raises(BillingError):
        await NullProvider().report_usage(customer_id="c", meter_event="m", quantity=1, identifier="i")
