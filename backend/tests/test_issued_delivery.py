...
def _inv(buyer, price, *, issue, due, email=None, penalty=None):
    body = {
        "buyer_name": buyer, "issue_date": issue, "due_date": due,
...
async def test_penalty_accrues_only_when_marked_and_overdue(auth_client):
    ...
    inv = (await auth_client.post("/api/v1/issued",
           json=_inv("Acme", "1000.00", issue="2025-12-01", due="2026-01-01", penalty="12"))).json()
    ...
    nop = (await auth_client.post("/api/v1/issued",
           json=_inv("Beta", "500.00", issue="2025-12-01", due="2026-01-01"))).json()
...
async def test_penalty_rate_inherits_issuer_default(auth_client):
    ...
    inv = (await auth_client.post("/api/v1/issued",
           json=_inv("Acme", "100.00", issue="2026-06-01", due="2026-06-15"))).json()
...
async def test_pdf_inline_and_period_zip(auth_client):
    ...
    await _activate(auth_client)
    a = (await auth_client.post("/api/v1/issued", json=_inv("Acme", "100.00", issue="2026-03-10", due="2026-04-10"))).json()
    await auth_client.post("/api/v1/issued", json=_inv("Beta", "200.00", issue="2026-03-20", due="2026-04-20"))
    await auth_client.post("/api/v1/issued", json=_inv("Gamma", "300.00", issue="2026-06-01", due="2026-07-01"))
...
async def test_send_invoice_records_outbox(auth_client):
    ...
    await _activate(auth_client)
    inv = (await auth_client.post("/api/v1/issued",
           json=_inv("Acme", "100.00", issue="2026-06-01", due="2026-06-30", email="ap@acme.io"))).json()
...
async def test_send_without_recipient_is_rejected(auth_client):
    pytest.importorskip("reportlab")
    await _activate(auth_client)
    inv = (await auth_client.post("/api/v1/issued", json=_inv("Acme", "100.00", issue="2026-06-01", due="2026-06-30"))).json()
    r = await auth_client.post(f"/api/v1/issued/{inv['id']}/send", json={})
...
async def test_reminder_single_and_bulk_overdue(auth_client):
    ...
    o1 = (await auth_client.post("/api/v1/issued", json=_inv("Acme", "100.00", issue="2025-12-01", due="2026-01-01", email="a@acme.io", penalty="10"))).json()
    (await auth_client.post("/api/v1/issued", json=_inv("Beta", "200.00", issue="2025-12-01", due="2026-01-01", email="b@beta.io"))).json()
    (await auth_client.post("/api/v1/issued", json=_inv("Gamma", "300.00", issue="2025-12-01", due="2026-01-01"))).json()  # no email
    ...
    paid = (await auth_client.post("/api/v1/issued", json=_inv("Delta", "50.00", issue="2026-06-01", due="2026-06-30", email="d@d.io"))).json()
...
async def test_receivables_report_includes_penalty(auth_client):
    await _activate(auth_client)
    await auth_client.post("/api/v1/issued", json=_inv("Acme", "1000.00", issue="2025-12-01", due="2026-01-01", penalty="12"))
