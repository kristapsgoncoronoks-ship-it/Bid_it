...
def test_readme_scale_numbers_match_the_live_tree():
    ...
    # Recounted at PROD-009: 111 `__tablename__` declarations across `app/models`
    # (WO-AB counted 108; WO-AG added `vat_country_requirements`, WO-AI
    # `archive_exports`, PROD-009 `workspace_exports`), matching
    # `Base.metadata.tables`. The literal is the tripwire — it is meant to be
    # re-derived when a table lands, never bumped to silence.
    assert readme_tables == 111, "table count claim moved — recount __tablename__ across app/models"
