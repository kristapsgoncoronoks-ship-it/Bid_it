...
async def _register(client, org, email):
    r = await client.post("/api/v1/auth/register", json={
        "organization_name": org, "name": "Owner", "email": email, "password": "supersecret"})
...
def _http_inv(n):
    return {
        "vendor_name": "SecretVendor", "invoice_number": f"A-{n}", "issue_date": "2026-07-01",
        "currency": "EUR", "status": "pending",
        "line_items": [{"description": "x", "category": "c", "quantity": "1", "unit_price": "100", "tax_rate": "0"}],
    }
...
async def test_fresh_registrant_sees_no_other_company_data(client):
    ...
    a_invoice_id = (await client.post("/api/v1/invoices", json=_http_inv(1), headers=_h(a_tok))).json()["id"]
    await client.put("/api/v1/modules/expenses", json={"enabled": True}, headers=_h(a_tok))
    a_report_id = (await client.post("/api/v1/expenses", json={"title": "A trip", "items": [
        {"spend_date": "2026-05-01", "category": "meals", "description": "Dinner", "amount": "50.00", "vat_amount": "0"}]},
    ...
    assert (await client.get(f"/api/v1/invoices/{a_invoice_id}", headers=_h(b_tok))).status_code == 404
    assert (await client.get(f"/api/v1/expenses/{a_report_id}", headers=_h(b_tok))).status_code == 404
    # B cannot modify A's resources either; A's data is untouched.
    assert (await client.delete(f"/api/v1/invoices/{a_invoice_id}", headers=_h(b_tok))).status_code == 404
    assert (await client.get("/api/v1/invoices", headers=_h(a_tok))).json()["total"] == 1
...
async def test_joining_a_company_requires_an_invitation(client):
    ...
    token = (await client.post("/api/v1/team/invites", json={"email": "staff@gamma.io", "role": "user"},
                               headers=_h(a_tok))).json()["token"]
