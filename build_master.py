...
def build(period=None):
    """Regenerate the master workbook Fleet_Fuel_Master_<period>.xlsx from the
    period-stamped consolidate pickle. Idempotent: re-running overwrites the file.

    Reads the pickle via consolidate.load_rows(period), which ASSERTS the pickle was
    written for `period` (reliability finding #3) before any work is done.

    IMPORTING this module is SIDE-EFFECT-FREE — the workbook is built only on this call
    (or via main()/__main__). Returns the output path.
    """
