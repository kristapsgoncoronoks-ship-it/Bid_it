...
def main():
    ...
    try:
        import backup
        p, n = backup.snapshot()
        ok(f"{os.path.basename(p)} ({n} files; nightly automation: see INSTALL.md Part 8)")
    except Exception as e:
...
{BOLD}=== Setup complete ==={END}
  ...
  Daily usage guide:  USER_MANUAL.md
  Team/server setup:  INSTALL.md (Parts 5-8: permanent service, proxy, backups)
""")
